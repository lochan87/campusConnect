...
const LoadingSkeleton = ({ type = 'post', count = 3 }) => {
  const SkeletonLine = ({ width, height = 'h-4', delay = 0 }) => (
    <motion.div 
      className={`${height} bg-gradient-to-r from-gray-200 via-gray-300 to-gray-200 rounded ${width}`}
      initial={{ opacity: 0.6 }}
  ...
  const PostSkeleton = ({ index }) => (
    ...
    >
      <div className="flex items-center space-x-4 mb-6">
        <motion.div 
          className="w-10 h-10 bg-gradient-to-r from-blue-200 to-purple-200 rounded-full"
  ...
  const PollSkeleton = ({ index }) => (
    ...
    >
      <div className="flex items-center space-x-4 mb-6">
        <motion.div 
          className="w-10 h-10 bg-gradient-to-r from-green-200 to-emerald-200 rounded-full"
    ...
    >
      <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        <div className="flex items-center space-x-3 mb-6">
          <motion.div 
            className="w-6 h-6 bg-gradient-to-r from-purple-200 to-violet-200 rounded"
          ...
          {[1, 2, 3].map((i) => (
            ...
            >
              <div className="flex items-center space-x-3 p-3 border border-gray-100 rounded-lg">
                <motion.div 
                  className="w-8 h-8 bg-gradient-to-r from-blue-200 to-indigo-200 rounded"
